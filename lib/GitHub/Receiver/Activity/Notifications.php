...
class Notifications extends AbstractActivity
{
     ...
     * @throws \Exception
     */
    public function listRepositoryNotifications(
        bool $all = false,
        bool $participating = false,
        string $since = 'now',
        string $before = null
        ...
        return $this->getApi()->request($this->getApi()->sprintf(
            '/repos/:owner/:repo/notifications?:args',
    ...
    {
        return $this->getApi()->request($this->getApi()->sprintf(
            '/notifications/threads/:id/subscription?:args',
            $id,
            http_build_query(['subscribed' => $subscribed, 'ignored' => $ignored])
        ), Request::METHOD_PUT);
    }
